def process():
    ...
    # Define the image size (92 pixels in width and 112 pixels in height)
    image_size = (92, 112)
    ...
    '''
    D = np.zeros((num_samples * images_per_sample, image_size[0] * image_size[1])) # row=400 , col=10304
    y = np.zeros(num_samples * images_per_sample) # labels 400
    
    
    # Process each subject's images
    for subject in range(1, num_samples + 1):
        for num in range(1, images_per_sample + 1):
            # Load the image
            ...
            '''
            index = (subject - 1) * images_per_sample + num - 1 
